...
- **세션별 우편함 주소 (2026-07-15 kyle 결정·실측 정정 — 다중 세션 지휘가 일상)**:
  ...
  - **지휘자 상주 도우미(표준)**: Monitor(persistent)로 `scripts/conductor-companion.sh <명패handle> <중계기handle> [순찰주기초=300]` 하나만 상주시킨다. 이 루프는 30초마다 `worker_done,escalation,decision_gate`를 확인하고, 시작 시점의 옛 편지는 베이스라인으로 제외하며, 새 메시지를 `SIGNAL` 한 줄로 출력한 뒤 종료하지 않는다. **실행자는 이 스크립트를 띄운 뒤 그 종료를 기다리지 않는다 (2026-07-27 실사고 — fire-and-forget)**: 판 수명 내내 상주하는 프로세스라 종료 대기는 곧 판 정체다. 백그라운드로 던진 직후 다음 단계(카드 분해·발령)로 진행한다. codex 하네스의 '백그라운드 터미널 종료 대기' 기능 등 하네스별 완료 대기에 넣는 것도 같은 이유로 금지. 같은 `taskId+dispatchId` 직접 신호는 한 번만 깨우고, relay가 복제한 정상 `worker_done`은 버린다. `status`는 읽음 처리만 해 과거 확인 답장이 입력창에 쌓이지 않게 한다. 기본 5분 순찰 주기마다 중계기에 `[순찰 알람]`을 보내고, `WATCH_DEADLINE_MIN`(기본 720분)이 지나면 `DEADLINE_REACHED` 후 종료한다. 따라서 `watch-inbox.sh` 수동 재가동과 별도 kicker는 표준 절차에서 불필요하다. **launchctl처럼 PATH가 축소된 실행 환경에서는 `ORCA_BIN=/usr/local/bin/orca`처럼 확인한 절대경로를 반드시 넘긴다.** 스크립트도 `/usr/local/bin/orca`, `/opt/homebrew/bin/orca`를 자동 탐색하고, 실행 파일을 못 찾으면 `ORCA_BIN_UNAVAILABLE`로 즉시 종료한다. 프로세스 생존만으로 성공 판정하지 말고 시작 후 `task-list/read`와 실제 wake 한 건을 끝단 확인한다.
  ...
  - **실제 기상 연결**: 프로젝트 감독이 Orca 터미널이면 `COMPANION_WAKE_TERMINAL_HANDLE=<프로젝트감독handle>`을 설정해 companion이 새 중요 신호를 같은 터미널 입력창에 `ORCA_INBOX_WAKE`로 직접 넣게 한다. 외부 프로그램을 깨우는 방식은 하네스별 어댑터가 companion의 `SIGNAL` stdout을 소비한다. 신호 출력만 존재하는 상태를 end-to-end 기상 성공으로 간주하지 않는다.
  - **구형 lifecycle 보고 실패 복구 (`[LEGACY READ-ONLY]`, 2026-07-31)**: 중계기나 작업자의 lifecycle 보고가 `legacy_read_only`·`effectsApplied=false`로 거부되면 같은 `escalation`·`worker_done`·`ask`를 재시도하거나 현재 명령으로 번역하지 않는다. 중계기는 작업자 터미널에 표식을 보내지 말고 `ORCA_LEGACY_READ_ONLY_REPORT task=<taskId> dispatch=<dispatchId> type=<보고종류> summary=<한줄요약>`을 **중계기 자신의 응답**에 한 줄 출력하고 끝낸다. companion은 중계기 화면과 현재 `dispatched` 작업자 화면을 읽기 전용으로 확인하며, 화면이 `BB`처럼 깨져 원문을 찾지 못할 때만 해당 dispatch의 최근 transcript를 폴백으로 읽는다. `taskId+dispatchId+type` 사건별로 한 번만 `COMPANION_WAKE_TERMINAL_HANDLE`의 **현재 프로젝트 감독**을 깨운다. companion과 중계기는 Run을 자동 인수하거나 카드·Dispatch 상태를 바꾸지 않는다. 원래 감독이 없거나 권한을 잃었다고 현재 감독이 확인한 경우에만, 현재 감독 자신의 살아 있는 터미널에서 공식 가이드의 `run-use --id <adopted_run_id> --takeover-legacy`를 실행한다. 이 환경변수가 구형 감독을 가리키면 복구가 성립하지 않으므로 판 재개 때 현재 handle로 companion을 다시 띄운다.
...
**라우팅 원장 자동 기록 (2026-07-27)**: `select-routing-pair.sh`(편성 선택)·`dispatch-safe.sh`(발령)·companion(worker_done)이 원장을 자동 append 한다 — 감독 수기는 검수 판정·랠리 총평만 남고, 체크포인트 커밋 턴에 같이 쓴다 (`references/routing-observability.md` 2층 구조). 판 세팅 시 dispatch-safe·companion 호출에 `ROUTING_BOARD=<판이름>`을 붙이면 원장 파일이 판 이름으로 모인다.
