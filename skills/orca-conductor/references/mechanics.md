...
- **세션별 우편함 주소 (2026-07-15 kyle 결정·실측 정정 — 다중 세션 지휘가 일상)**:
  ...
  - **구형 중계기 보고 실패 복구 (`[LEGACY READ-ONLY]`, 2026-07-31)**: 중계기의 lifecycle 보고가 `legacy_read_only`·`effectsApplied=false`로 거부되면 같은 `escalation`·`worker_done`·`ask`를 재시도하거나 현재 명령으로 번역하지 않는다. 중계기는 `ORCA_LEGACY_READ_ONLY_REPORT task=<taskId 또는 unknown> dispatch=<dispatchId 또는 unknown> type=<보고종류> summary=<한줄요약>`을 자기 터미널에 한 줄 출력하고 끝낸다. companion은 중계기 화면을 읽기 전용으로 확인해 같은 표식을 한 번만 처리하고 `COMPANION_WAKE_TERMINAL_HANDLE`의 **현재 프로젝트 감독**을 깨운다. companion과 중계기는 Run을 자동 인수하거나 카드·Dispatch 상태를 바꾸지 않는다. 원래 감독이 없거나 권한을 잃었다고 현재 감독이 확인한 경우에만, 현재 감독 자신의 살아 있는 터미널에서 공식 가이드의 `run-use --id <adopted_run_id> --takeover-legacy`를 실행한다. 이 환경변수가 구형 감독을 가리키면 복구가 성립하지 않으므로 판 재개 때 현재 handle로 companion을 다시 띄운다.
