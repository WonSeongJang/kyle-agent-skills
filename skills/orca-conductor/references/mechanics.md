...
## 0) ★★★ 핵심 원칙 — 추측하지 말고 도구로 뽑아라 (2026-08-11 kyle 확정, 이 하네스의 키) ★★★

**모든 운영 개념은 세 표면을 갖는다: 원본 파일 하나 + 사람 화면 하나 + 에이전트 창구 하나.**

```
상태    대시보드 화면        =  curl -s http://127.0.0.1:8787/status.txt
정책    라우팅 탭            =  curl -s http://127.0.0.1:8787/api/routing
규칙    규칙 탭              =  curl -s http://127.0.0.1:8787/rules.txt
성적    (성적 탭 예정)       =  .orca/routing-events/<판>.jsonl (card_outcome)
```
