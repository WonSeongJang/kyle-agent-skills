...
- **세션별 우편함 주소 (2026-07-15 kyle 결정·실측 정정 — 다중 세션 지휘가 일상)**:
  ...
  - relay는 super upper report·kyle 질문·카드 상태 변경을 직접 하지 않는다. kicker는 relay에 도구 실행 줄·Context% 증가, 연속 무진행 2회 정체 보고, bounded 후보 구조화 지시를 보낸다. Context 50% 또는 판 경계에서는 새 relay 세션으로 교대한다.
