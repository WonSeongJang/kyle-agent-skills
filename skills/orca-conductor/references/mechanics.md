...
- **순찰 에이전트(patrol)**: 판에 일꾼 터미널이 3개 이상이거나 30분 이상 걸리는 장기 카드가 돌 때 편성한다. `codex-luna-low` 터미널 1개를 쓰며, 레포 루트에서 실행해도 되고 워크트리는 필요 없다. 순찰 카드 spec: `[읽기 전용] 5분 주기로 지정 터미널들을 읽어 진행중/정체/사망을 판정한다. 근거는 도구 실행 줄과 Context% 증가만 인정하고 스피너는 인정하지 않는다(2026-07-19 사고). 같은 활성 터미널에서 진행 증거가 2회 연속 없으면 정체로 보고한다. 명확한 오류·확인창·프리징·프로세스 종료는 첫 발견에 즉시 보고한다. 정상 진행은 통신기록에만 남기고 발신하지 않는다. 지휘자의 판 종료 지시까지 반복한다.`
  - 역할 분담: `scripts/watch-terminals.sh`는 알려진 오류 문구를 토큰 0으로 감시하고, 순찰은 판단이 필요한 정체를 저비용 `luna-low`로 판정한다. 비용 원칙은 조용한 감시이며, 이상일 때만 발신한다.
