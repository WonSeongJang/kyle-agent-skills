...
- **새 세션 의무 — 값을 숫자로 쟀다 (EXP-RI-1, 2026-08-10)**: 검수자가 구현 문맥을 이어받으면 안 판다. 같은 커밋·같은 실행기(omo)·같은 모델(gpt-5.6-terra)·같은 노력(high)으로 검수자의 문맥 독립성만 바꿔 쟀다.

  | | 팔 A 새 세션 | 팔 B 구현 문맥 이어받음 | 심판 (다른 모델 sol) |
  |---|---:|---:|---:|
  | 진짜 결함 발견 | 1 | 0 | 2 |
  | 놓침 | 1 | 2 | 0 |
  | 오탐 | 0 | 0 | 0 |
  | 걸린 시간 | 314.1초 | 153.6초 | 597초 |
  | 띄운 서브에이전트 | 0 | 0 | 5 |

  팔 B는 자기 구현이 만족시킨 요구사항을 다시 확인하는 데 그쳤고 입력 타입 계약과 시험의 실패 원인은 공격하지 않았다. **한 표본이라 원인이 문맥인지 검수 시간인지 우연인지는 못 가른다** — 확정하지 말 것. 다만 자체 검수만 믿었으면 중요 결함 2개가 통과했다. 원본: `/Users/fw_m1/Dev/conductor-core/.staging/result-EXP-RI-1.md`
- **검수 시간 기준선 (같은 시험에서 얻음)**: `153.6초 → 0개` / `314.1초 → 1개` / `597초 → 2개`. 이 판의 첫 기준선이다. 검수가 2~3분 만에 CODE_PASS로 돌아오면 "빠르다"가 아니라 "덜 팠다"를 먼저 의심하고, 무엇을 실행했는지 근거 목록부터 본다. 표본이 쌓이면 이 표를 갱신한다.
- **심판 층 (제3자) — 쓸 때와 안 쓸 때**: 두 검수 결과의 합집합을 정답지로 삼으면 **둘 다 놓친 것은 영원히 안 보인다.** EXP-RI-1에서 A·B가 함께 놓친 중요 결함 1건을 심판만 찾았다 — 심판이 없었으면 "둘 다 괜찮다"로 끝났다. 심판은 A·B 결과를 보지 않은 채 같은 커밋을 처음부터 검수한다. **상시 쓰지 않는다** — 검수 방식 자체를 비교하는 시험, 합격 판정을 신뢰할 수 없는 국면, kyle 지시일 때만. 판정 절차는 배틀 오프닝(§배틀)의 점수표·결정 관문 방식을 그대로 쓴다.
- **보고 계약 (2026-08-02 kyle 결정, 원본 링크)**: 구현·수정·검수 카드는 완료 시 현재 Run에 `worker_done`으로 보고하고, 프로젝트 감독은 슈퍼 감독의 공식 Run으로 상위 보고한다. 계약 원본(전체 규칙·구조화 형식·malformed 격리·결정 관문 해결 기준)은 `~/.claude/skills/super-conductor/references/adapter-contract.md`의 "보고 계약" 절이다.
