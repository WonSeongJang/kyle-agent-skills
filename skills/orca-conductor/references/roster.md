...
- **effort 상향 실험 (2026-08-04 kyle 결정 — "추론 좀 올려보려고")**: 기본 강도는 위 표 그대로 두고, 추론 강도를 올린 **실험 항목**을 `routing-providers.json`에 추가 등록했다. dev: `codex-glm52-max`·`codex-kimi1m-max` (각 20%, 검수는 openai·anthropic 계열만 허용). 검수: `codex-sol-high/xhigh/max` (각 20%, **medium 기준선 항목은 그대로 유지** — 원장의 effort 필드로 A/B 비교), `codex-glm52-max` 검수 (20%). `claude-fable-medium/high/xhigh`는 실험이 아니라 **최후 보루 전용(`lastResortOnly`)** — 정상 조합이 전부 불능·예약선 침범일 때만 자동 선발된다. 선택기의 실험 게이트는 developer뿐 아니라 **reviewer에도 적용**되며(2026-08-04 코드 반영), 실험 버킷은 `모델 id + effort` 단위로 독립이라 같은 모델의 단계별 실험이 서로 다른 카드에서 열린다. 예상 노출(sol 검수): max 20% / xhigh 16% / high 12.8% / medium 51.2%.
