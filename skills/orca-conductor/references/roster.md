...
| `claude-opus5-<강도>` | `claude --model opus --effort <강도> --dangerously-skip-permissions` | low·medium·high·xhigh·max | **기동 검증됨 (2026-07-25 실측)** · **작업자 전용 전환 (2026-08-04 kyle 결정)** — reviewer 항목 `enabled: false`(Anthropic 계열 검수는 fable), dev는 medium/high/xhigh 3단 각 20% 실험으로 운용 (검수는 openai 계열만) |
| `omo-sol-<강도>` | `omo --model openai-codex/gpt-5.6-sol --thinking <강도> --no-recommended-models` | low·medium·high (thinking 스케일) | **감독 러너 정식 등록 (2026-08-12 kyle 승인)** — 첫 실전 판 skills-repo-migration-1 감독 완주(카드 12장·작업자 exit -1 자체 복구·검수 FAIL 후속 종결·표준 종료 준수). 특수 규칙: **터미널 주입은 한 줄만**(여러 줄 주입 금지), **모델 자칭 불신 — 세션 파일(`~/.omo/sessions/`)로 검증**, `--no-recommended-models` 플래그 필수(없으면 kimi로 갈아탐). 작업자 실적: dashboard-single-source-1 무결함 1라운드. 쿼터는 openai 계열 공유 |
...
- **effort 상향 실험 (2026-08-04 kyle 결정 — "추론 좀 올려보려고")**: 기본 강도는 위 표 그대로 두고, 추론 강도를 올린 **실험 항목**을 `routing-providers.json`에 추가 등록했다. dev: `codex-glm52-max`·`codex-kimi1m-max` (각 20%, 검수는 openai·anthropic 계열만 허용), `claude-opus5-medium/high/xhigh` (각 20%, 검수는 openai 계열만 — opus 작업자 전용 전환). luna-max 실험의 검수 제한은 anthropic 단독에서 openai·anthropic으로 완화 — 루나 작업을 sol이 검수할 수 있고, 자기 모델 검수(루나-루나, 솔-솔)는 선택기가 원천 차단한다 (2026-08-04 kyle 결정). 검수: `codex-sol-high/xhigh/max` (각 20%, **medium 기준선 항목은 그대로 유지** — 원장의 effort 필드로 A/B 비교), `codex-glm52-max` 검수 (20%), `codex-kimi1m-max` 검수 (20%, 품질 75 — glm 검수(72~74)와 경쟁하는 단계로 상향, 2026-08-04 kyle 결정). sol이 같은 계열 감점을 받는 luna 작업 카드나 openai 불능 국면에서 glm 대신 뽑히며 표본을 모은다. sol 기준선(98)과의 직접 경쟁은 원장 표본 확인 후 다음 단계. 기준선 `codex-kimi1m-high`의 낮은 품질(20)은 그대로 유지. `claude-fable-medium/high/xhigh`는 실험이 아니라 **최후 보루 전용(`lastResortOnly`)** — 정상 조합이 전부 불능·예약선 침범일 때만 자동 선발된다. 선택기의 실험 게이트는 developer뿐 아니라 **reviewer에도 적용**되며(2026-08-04 코드 반영), 실험 버킷은 `모델 id + effort` 단위로 독립이라 같은 모델의 단계별 실험이 서로 다른 카드에서 열린다. 예상 노출(sol 검수): max 20% / xhigh 16% / high 12.8% / medium 51.2%.
- **자기 모델 검수 차단 — 코드로 확인함 (2026-08-10 실측)**: 위 "선택기가 원천 차단한다"는 글만이 아니라 실제로 들어가 있다. `scripts/select_routing_pair.py:337` 의 `if dev_model.id == review_model.id: continue` 가 후보 생성 단계에서 걸러낸다. `id` 는 모델 이름이고 `effort` 는 별도 필드라, 노력이 달라도 같은 모델이면 걸린다.
  - 실측: 작업 종류 8종 × 실험 키 10개 = **선발 80회, 같은 모델 0회 / 같은 계열 0회.** 후보 목록에 오른 3,318쌍 중에도 같은 모델은 0쌍이었다 — 뽑히고 나서 감점되는 게 아니라 애초에 목록에 못 들어온다.
  - **같이 걸린 것 (미해결, 별건)**: 그 80회에서 검수자가 **80회 전부 `gpt-5.6-sol`** 이었다. 구현자도 `zai/glm-5.2` 42회 · `opus` 38회 둘뿐이고 kimi·luna 는 developer 로 한 번도 안 뽑혔다. 자기 모델 검수는 막혔지만 **검수자 단일화**가 그 자리를 채우고 있다. 편성 쏠림은 이미 알려진 항목(2026-08-09 기록)이며, 이 실측이 그 증거를 하나 더한다. 지금 고치지 않는다 — 원인이 품질값인지 쿼터 미수집인지 갈라야 한다.
...
- **terra 작업자 라우터 오프 (2026-08-04 kyle 결정)**: `routing-providers.json`의 terra developer 항목을 `enabled: false`로 껐다 — 항목·품질값은 보존되고, 재활성은 플래그 하나만 되돌리면 된다. 모델 단위 온오프는 이 `enabled` 플래그가 표준(미기재 시 기본 켜짐). 수동 프리셋(`codex-terra-*`)은 비상용으로 로스터에 남는다.
...
외부 사례·표준을 찾는 선행 조사는 `research-flow.md` 계약을 추가 적용한다. **조사 기본은 glm-5.2 medium이다 (2026-08-11 kyle — zai는 5시간 리셋뿐이라 주간 고갈이 없고, kimi는 보존한다). 대규모 읽기도 glm 기본이다 — glm도 1M 문맥이다(kyle 확인, 로컬 실측 불가). luna max·terra high도 조사 후보로 쓸 수 있다(2026-08-11 kyle — 구 '루나는 링크 수집 보조' 제한 해제, terra 재활성). Kimi-low는 위 후보 소진 시의 폴백이다.** 큰 표준이나 여러 원본 저장소를 비교할 때만 high로 올린다. (구 기본 Terra-low·Kimi-low는 폐기 — Terra는 꺼져 있어 사실상 Kimi 단독 기본이었고, 그래서 조사가 전부 kimi로 갔다.) Luna는 최종 비교 판단자가 아니라 링크 수집 보조로만 쓴다.
