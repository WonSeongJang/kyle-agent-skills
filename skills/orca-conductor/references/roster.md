...
| `claude-fable-<강도>` | `claude --model fable --effort <강도> --dangerously-skip-permissions` | low·medium·high | 검증됨 · 최후순위 전용 |
...
- **선택기에 등록되지 않은 비상 검수자**: 등록된 조합이 모두 불능일 때만 `claude-fable-medium`을 사람이 고르는 최후 보루로 쓴다. 평상시 Kimi/Sol 검수 순서는 고정하지 않고 선택기 점수로 정한다. Fable을 정식 후보로 운영하게 되면 `routing-providers.json`에 provider와 모델을 등록한다.
